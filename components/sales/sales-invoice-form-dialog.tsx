...
export function SalesInvoiceFormDialog({
  ...
  onSaved: () => void;
  /** Used to persist driver/vehicle pick lists in localStorage per org. */
  ...
  }, [open, initialInvoice, initialLines, productLookup.byId, customers]);

  /** Load driver/vehicle pick lists from DB (shared org-wide) merged with invoice hints. */
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    (async () => {
      ...
      const [dRes, vRes] = await Promise.all([
        getMergedPicklist(organizationId, PICKLIST_KEYS.salesInvoiceDriver, driverOptions),
        getMergedPicklist(organizationId, PICKLIST_KEYS.salesInvoiceVehicle, vehicleOptions),
      ]);
      if (cancelled) return;
  ...
  }, [open, organizationId, driverOptions, vehicleOptions]);
    ...
    >
      ...
      <ProductStyleSearchBlobDialog
        ...
        onItemsChange={(next) => {
          ...
          if (organizationId) {
            void savePicklist(organizationId, PICKLIST_KEYS.salesInvoiceDriver, next, driverOptions);
          }
        }}
      ...
      <ProductStyleSearchBlobDialog
        ...
        onItemsChange={(next) => {
          ...
          if (organizationId) {
            void savePicklist(organizationId, PICKLIST_KEYS.salesInvoiceVehicle, next, vehicleOptions);
          }
        }}
