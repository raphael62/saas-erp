...
import { getMergedPicklist, savePicklist } from "@/lib/organization-picklist-actions";
import { PICKLIST_KEYS } from "@/lib/organization-picklist-keys";
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [tRes, dRes, vRes] = await Promise.all([
        getMergedPicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceTransporter, transporterOptions),
        getMergedPicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceDriver, driverOptions),
        getMergedPicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceVehicle, vehicleOptions),
      ]);
      if (cancelled) return;
      setTransporterLookupItems(tRes.error ? transporterOptions : tRes.values);
      setDriverLookupItems(dRes.error ? driverOptions : dRes.values);
      setVehicleLookupItems(vRes.error ? vehicleOptions : vRes.values);
    })();
    return () => {
      cancelled = true;
    };
  }, [open, organizationId, transporterOptions, driverOptions, vehicleOptions]);
  ...
  return (
    ...
    >
      ...
      <ProductStyleSearchBlobDialog
        ...
        onItemsChange={(next) => {
          setTransporterLookupItems(next);
          void savePicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceTransporter, next, transporterOptions);
        }}
      ...
      <ProductStyleSearchBlobDialog
        ...
        onItemsChange={(next) => {
          setDriverLookupItems(next);
          void savePicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceDriver, next, driverOptions);
        }}
      ...
      <ProductStyleSearchBlobDialog
        ...
        onItemsChange={(next) => {
          setVehicleLookupItems(next);
          void savePicklist(organizationId, PICKLIST_KEYS.purchaseInvoiceVehicle, next, vehicleOptions);
        }}
